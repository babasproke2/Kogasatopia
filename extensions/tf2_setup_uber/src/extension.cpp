 ...
 *
 * Detours CWeaponMedigun::HealTargetThink(), lets the stock function run, then
 * rescales the charge delta it produced during setup time.
 */
#include "extension.h"
...
static CDetour *g_pHealTargetThinkDetour = NULL;
...
static bool IsSetupActive()
{
    ...
    return ReadBoolField(pGameRules, g_iGameRulesInSetupOffset);
}
...
DETOUR_DECL_MEMBER0(CWeaponMedigun_HealTargetThink, void)
{
    ...
    const float flDelta = flAfter - flBefore;
     ...
     *
     * If stock has already capped to 100% and the requested value is >= 3.0,
     * leaving it capped is already correct.
     */
    ...
    const float flNewCharge = ClampFloat(flBefore + flDelta * flScale, 0.0f, 1.0f);
...
bool TF2SetupUberExt::SDK_OnLoad(char *error, size_t maxlength, bool late)
{
    ...
    g_pHealTargetThinkDetour = DETOUR_CREATE_MEMBER(CWeaponMedigun_HealTargetThink, "CWeaponMedigun::HealTargetThink");
    if (g_pHealTargetThinkDetour == NULL)
    {
        snprintf(error, maxlength, "Could not create detour for CWeaponMedigun::HealTargetThink; update gamedata signature");
        return false;
    }

    g_pHealTargetThinkDetour->EnableDetour();
    g_bDetourReady = true;
...
void TF2SetupUberExt::SDK_OnUnload()
{
    ...
    if (g_pHealTargetThinkDetour != NULL)
    {
        if (g_pHealTargetThinkDetour->IsEnabled())
            g_pHealTargetThinkDetour->DisableDetour();

        g_pHealTargetThinkDetour->Destroy();
        g_pHealTargetThinkDetour = NULL;
    }
...
bool TF2SetupUberExt::QueryRunning(char *error, size_t maxlength)
{
    if (!g_bDetourReady)
    {
        snprintf(error, maxlength, "CWeaponMedigun::HealTargetThink detour is not ready");
        return false;
    }
