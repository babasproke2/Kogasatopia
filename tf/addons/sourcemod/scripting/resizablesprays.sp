...
public void OnPluginStart()
{
	...
	cv_bChatty = CreateConVar("rspr_chatty", "1", "Whether to print resizable sprays chat notifications.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cv_iLogLevel = CreateConVar("rspr_loglevel", "2", "Logging level. Higher number = more console spam.", FCVAR_NONE, false, 0.0, false, 0.0);
	cv_fMaxSprayDistance = CreateConVar("rspr_maxspraydistance", "1024.0", "Max range for placing decals. 0 is infinite range", FCVAR_NOTIFY, true, 0.0, false);
	cv_fMaxSprayScale = CreateConVar("rspr_maxsprayscale", "240", "Maximum scale for sprays for regular players.", FCVAR_NOTIFY, true, 0.0, false, 0.0);
	...
	cv_fSprayTimeout = CreateConVar("rspr_spraytimeout", "20.0", "Max time to wait for clients to download spray files. 0 to wait forever.", FCVAR_NOTIFY, true, 0.0, false);
...
public void OnClientPostAdminCheck(int client)
{
	...
	if (cv_bChatty.BoolValue)
		PrintToChat(client, "[Kogasa] Preparing your spray for resizing...");
...
public Action Timer_CheckIfSprayIsReady(Handle timer, int client)
{
	...
	if (g_Players[client].bSprayHasBeenProcessed && !g_Players[client].bIsReadyToSpray && GetSprayQueueCount(client) <= 0) {
		...
		if (cv_bChatty.BoolValue)
			PrintToChat(client, "[Kogasa] Your spray is ready! Type /spray %d to make big sprays.", RoundToZero(cv_fMaxSprayScale.FloatValue));
		return Plugin_Stop;
	} else {
	...
	if (!isSprayMaterial) {
		if (StrEqual(filename[strlen(filename)-4], ".dat") && client > 0) {
			...
			if (IsValidClient(client) && g_Players[client].bTriedToSprayWhenDownloadingVtfs) {
				g_Players[client].bTriedToSprayWhenDownloadingVtfs = false;
				if (cv_bChatty.BoolValue)
					PrintToChat(client, "[Kogasa] Your client has finished downloading spray files.");
			}
...
public Action Command_Spray(int client, int args)
{
	...
	if (args > 0) {
		...
		if (IsAdmin(client) && args == 2) {
			...
			if (!IsValidClient(spray.iOwner)) {
				ReplyToCommand(client, "[Kogasa] This client isn't in game yet! Please try again later.");
				return Plugin_Handled;
			}
		}
	}
	...
	if (!g_Players[spray.iOwner].bIsReadyToSpray) {
		ReplyToCommand(client, "[Kogasa] We're still preparing this spray! Please try again later.");
		return Plugin_Handled;
	}
	...
	if (!g_MaterialMap.GetArray(vmtFilename, material, sizeof(material))) {
		...
		if (queue > 0) {
			...
			ReplyToCommand(client, "[Kogasa] Your client is currently downloading %d spray files. Please try again later.", queue);
			return Plugin_Handled;
		}
	}
	...
	if (spray.iEntity > -1) {
		...
		ReplyToCommand(client, "[Kogasa] You are too far away from a valid surface to place a spray!");
	}
...
public Action Command_SprayInfo(int client, int args)
{
	...
	if (strlen(sMessage) > 128) {
		if (cv_bChatty.BoolValue)
			PrintToChat(client, "[Kogasa] See console for output.");
