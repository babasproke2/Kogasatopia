...
public void PowerfulHits_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		PowerfulHits_ApplyPerk(client, perk);
	}
	else
	{
		PowerfulHits_RemovePerk(client, reason);
	}
}
