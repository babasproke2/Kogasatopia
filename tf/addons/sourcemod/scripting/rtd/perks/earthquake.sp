...
public void Earthquake_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		Earthquake_ApplyPerk(client, perk);
	}
	else
	{
		Earthquake_RemovePerk(client, reason);
	}
}
