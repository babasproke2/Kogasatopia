...
#define Crits Int[0]
public void HomingProjectiles_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		HomingProjectiles_ApplyPerk(client, perk);
	}
	else
	{
		HomingProjectiles_RemovePerk(client, reason);
	}
}
