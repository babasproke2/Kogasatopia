...
#define Base Float[0]
public void StrongGravity_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		StrongGravity_ApplyPerk(client, perk);
	}
	else
	{
		StrongGravity_RemovePerk(client, reason);
	}
}
