...
public void DrunkWalk_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		DrunkWalk_ApplyPerk(client, perk);
	}
	else
	{
		DrunkWalk_RemovePerk(client, reason);
	}
}
