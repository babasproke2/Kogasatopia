...
#define Ticks Int[0]
public void WeaponMixup_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		WeaponMixup_ApplyPerk(client, perk);
	}
	else
	{
		WeaponMixup_RemovePerk(client, reason);
	}
}
