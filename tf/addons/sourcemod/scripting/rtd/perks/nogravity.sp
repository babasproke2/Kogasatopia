...
#define Base Float[0]
public void NoGravity_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		NoGravity_ApplyPerk(client, perk);
	}
	else
	{
		NoGravity_RemovePerk(client, reason);
	}
}
