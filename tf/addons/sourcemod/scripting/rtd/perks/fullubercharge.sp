...
public void FullUbercharge_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		FullUbercharge_ApplyPerk(client, perk);
	}
	else
	{
		FullUbercharge_RemovePerk(client, reason);
	}
}
