...
#define Boost Int[0]
public void Criticals_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		Criticals_ApplyPerk(client, perk);
	}
	else
	{
		Criticals_RemovePerk(client, reason);
	}
}
