...
public void Lag_Call(const int client, const Perk perk, const bool apply, const RTDRemoveReason reason)
{
	if (apply)
	{
		Lag_ApplyPerk(client, perk);
	}
	else
	{
		Lag_RemovePerk(client, reason);
	}
}
