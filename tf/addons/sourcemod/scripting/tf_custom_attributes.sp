 ...
 */
#define MAX_ATTRIBUTE_TOKEN 16000000

StringMap g_AttributeKVs;
int g_NextAttributeToken = 1;
...
public void OnPluginStart() {
	...
	g_AttributeKVs = new StringMap();
	
	ConVar version = CreateConVar("tf2custattr_version", PLUGIN_VERSION, .flags = FCVAR_NOTIFY);
...
public void OnPluginEnd() {
	...
	delete g_AttributeKVs;
	delete g_OnAttributeKVAdded;
}
...
public void OnItemAttributeSpawnPost(int entity) {
	if (!GetCustomAttributeStruct(entity, .validate = true)) {
...
public Action GarbageCollectAttribute(Handle timer) {
	StringMapSnapshot attributeTokens = g_AttributeKVs.Snapshot();
	...
	while ((entity = FindEntityByClassname(entity, "*")) != -1) {
		...
		int token = GetCustomAttributeToken(entity);
		if (token > 0) {
			char tokenKey[16];
			IntToString(token, tokenKey, sizeof(tokenKey));
			if (g_AttributeKVs.ContainsKey(tokenKey)) {
	...
	for (int i = 0; i < attributeTokens.Length; i++) {
		...
		KeyValues kv;
		if (g_AttributeKVs.GetValue(tokenKey, kv) && IsValidHandle(kv)) {
			delete kv;
		}
		g_AttributeKVs.Remove(tokenKey);
	}
...
int GetCustomAttributeToken(int entity) {
	...
	if (token <= 0 || token > MAX_ATTRIBUTE_TOKEN
...
KeyValues GetCustomAttributeStruct(int entity, bool validate) {
	...
	KeyValues kv;
	if (g_AttributeKVs.GetValue(tokenKey, kv) && (!validate || IsValidHandle(kv))) {
		return kv;
	}
...
void SetCustomAttributeStruct(int entity, KeyValues kv) {
	int token = AllocateCustomAttributeToken();
	char tokenKey[16];
	IntToString(token, tokenKey, sizeof(tokenKey));
	g_AttributeKVs.SetValue(tokenKey, kv);
	TF2Attrib_SetByDefIndex(entity, ATTRID_CUSTOM_STORAGE, float(token));
}

int AllocateCustomAttributeToken() {
	for (int attempt = 0; attempt < MAX_ATTRIBUTE_TOKEN; attempt++) {
		int token = g_NextAttributeToken++;
		if (g_NextAttributeToken > MAX_ATTRIBUTE_TOKEN) {
			g_NextAttributeToken = 1;
		}
