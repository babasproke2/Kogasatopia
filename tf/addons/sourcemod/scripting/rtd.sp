...
public Action Event_PlayerDeath(Event hEvent, const char[] sEventName, bool dontBroadcast)
{
	...
	{
		PointsStore_ApplyBonusPoints(attacker, "Killed positive RTD player", true, true, 1.0, client, 3.0);
	}
...
bool SpendRollCost(int client)
{
	...
	if (PointsStore_GetBonusPoints(client) < cost)
	{
		PrintRollCostNeeded(client, cost);
		return false;
	}
...
bool IsPointsStoreCostAvailable()
{
...
void PrintRollCostNeeded(int client, int cost)
{
	char currencyColor[34];
	char currencyName[64];
	GetRollCurrencyColor(currencyColor, sizeof(currencyColor));
	...
	CPrintToChat(client, CHAT_PREFIX ... " You need %s%d %s{default}to roll the dice!", currencyColor, cost, currencyName);
}

void GetRollCurrencyName(char[] buffer, int maxlen)
{
	ConVar currencyName = FindConVar("sm_points_store_currency_long");
	if (currencyName != null)
	{
		currencyName.GetString(buffer, maxlen);
		TrimString(buffer);
	}
	else
	{
...
void GetRollCurrencyColor(char[] buffer, int maxlen)
{
	char color[32];
