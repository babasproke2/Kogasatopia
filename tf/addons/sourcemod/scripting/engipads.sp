...
public Plugin myinfo =
{
	...
	author 			= "Starblaster 64, attribute and other new features made by Hombre",
...
public void OnPluginStart()
{
    ...
    RegConsoleCmd("sm_sakuya", TogglePadsMenuCmd);
...
cvarPads[PadForce] = CreateConVar("sm_engipads_force", "0", "0 - Nothing 1- Force Pads 2 - Force no pads", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
cvarPads[BoostBlockAiming] = CreateConVar("pads_boost_block_aiming", "1", "Set to 1 to prevent scoped-in/revved up players from being speed boosted.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	//Cookies
	g_hPadCookie = RegClientCookie("engipads_toggle", "Client's EngiPads toggle preference.", CookieAccess_Public);
...
public Action TogglePadsMenuCmd(int iClient, int nArgs)
{
        ...
        if (GetClientPadsEnabled(iClient))
        {
                SetClientPadsEnabled(iClient, false);
                CPrintToChat(iClient, "{orange}[EngiPads]{default} %t", "padphrase_disabled");
        } else {
                SetClientPadsEnabled(iClient, true);
